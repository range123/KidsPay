...
def transfer_to_merchant(*,
    ...
    if transaction_in.amount > child.max_single_transaction_limit:
        raise HTTPException(status_code=400, detail="Amount exceeds restriction set")
    transaction_id = uuid.uuid1().int>>34
    try:
        paypal_admin.make_payment_to_receiver(transaction_in.receiver_id, transaction_id, transaction_in.amount, child.name)
    except:
...
def transfer_to_child(*,
    ...
    other_child : models.Child = crud.child.get_by_username(db,transaction_in.receiver_id)
    if not other_child:
        raise HTTPException(status_code=400, detail="Receiver child does not exist")
    crud.child.withdraw_balance(db, child.id, transaction_in.amount)
